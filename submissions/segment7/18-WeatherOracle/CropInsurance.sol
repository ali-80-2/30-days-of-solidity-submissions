...
contract CropInsurance is Ownable {
    ...
    constructor(address _weatherOracle, address _ethUsdPriceFeed) payable Ownable(msg.sender) {
        weatherOracle = AggregatorV3Interface(_weatherOracle);//the address of our rainfall oracle (like the mock we built earlier).
        ethUsdPriceFeed = AggregatorV3Interface(_ethUsdPriceFeed);//the address of a Chainlink price feed that gives us ETH → USD conversion.
    }
...
*/
    function checkRainfallAndClaim() external {
        ...
        uint256 currentRainfall = uint256(rainfall);//Converts rainfall to unsigned format.
